/*
   Copyright ?-2019 https://sourced.tech
   Copyright holders of this repo https://github.com/src-d/go-mysql-server
...
// WriteValues writes the main node.
func (p *TreePrinter) WriteValues(args ...interface{}) error {
	if p.nodeWritten {
		return ErrNodeAlreadyWritten
	}
...
// WriteNlf writes the main node.
func (p *TreePrinter) WriteNlf(format string, args ...interface{}) error {
	if p.nodeWritten {
		return ErrNodeAlreadyWritten
	}

	fmt.Fprintf(&p.buf,"%s{"/*}*/,format)
